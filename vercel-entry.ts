...
// ─── API handler (reuse the same routing as serve.ts) ──────────────────
import {
  ...
  try {
    if (existsSync(fullPath)) {
      ...
      return new Response(content, {
        headers: { "Content-Type": getMimeType(fullPath), "Cache-Control": "public, max-age=3600" },
      });
    }
  } catch {}
...
// ─── API routing (mirrors serve.ts) ────────────────────────────────────
async function handleApi(req: Request): Promise<Response | null> {
  const url = new URL(req.url);
  const path = url.pathname;
  const method = req.method;
  if (!path.startsWith("/api/") && !path.startsWith("/tip/@")) return null;

  try {
    // ─── Tip landing page ───
    const tipPageMatch = path.match(/^\/tip\/@([a-zA-Z0-9_-]+)$/);
    if (tipPageMatch) return handleTipLandingPage(req, tipPageMatch[1]);
    ...
    if (deviceMatch && method === "DELETE") return handleDeviceDelete(req, deviceMatch[1]);
...
export default async function vercelHandler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  try {
    ...
    // Try API first
    let webRes = await handleApi(webReq);
    if (!webRes) {
      // Serve static file or SPA fallback
      const url = new URL(webReq.url);
      webRes = serveStatic(url.pathname) ?? new Response("Not found", { status: 404 });
    }
    ...
    console.error("[catalog] Vercel handler error:", error);
    res.statusCode = 500;
